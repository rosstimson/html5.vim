...
" GIT:  http://github.com/ChrisYip/Better-CSS-Syntax-for-Vim
...
syn keyword cssPseudo link visited active hover focus left right root empty target enabled disabled checked indeterminate optional valid invalid required
...
syn match cssProp /\(appearance\|azimuth\|binding\|bottom\|clear\|clip\|color\|columns\|content\|crop\|cursor\|direction\|elevation\|empty-cells\|hanging-punctuation\|height\|hyphens\|icon\|inline-box-align\|left\|letter-spacing\|move-to\|opacity\|orphans\|phonemes\|pointer-events\|position\|play-during\|presentation-level\|punctuation-trim\|quotes\|rendering-intent\|resize\|richness\|right\|size\|speech-rate\|stress\|string-set\|tab-size\|table-layout\|top\|unicode-bidi\|vertical-align\|visibility\|volume\|widows\|width\|z-index\)\ze\s*:/ contained

syn match cssProp /\(\<\|\)transform\(-\(origin\|style\)\)\=\>\ze\s*:/ contained
...
syn match cssProp /\(\<\|\)border\(-\(bottom\|collapse\|color\|image\(-\(outset\|repeat\|slice\|source\|width\)\)\=\|left\|length\|radius\|right\|spacing\|style\|top\|width\)\)\{0,1\}\>\ze\s*:/ contained
syn match cssProp /\(\<\|\)border\(-\(bottom\|left\|right\|top\)\(-\(color\|style\|width\)\)\{0,1\}\)\{0,1\}\>\ze\s*:/ contained
